...
Machine.prototype.getCallStack = function () {
  var stack = [];
  var stackFrame = this.$runner.gen.stackFrame;
  if (stackFrame) {
    stack.push(clone(this.$runner.gen.stackFrame));
  }
  for (var i = this.$runner.stack.length - 1; i >= 0; i--) {
    stackFrame = this.$runner.stack[i].stackFrame;
    if (stackFrame) {
      stack.unshift(clone(stackFrame));
    }
  }
  return stack;
};
